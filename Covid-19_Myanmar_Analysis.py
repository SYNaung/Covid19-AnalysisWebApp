...
try:
    ...
    st.subheader("Today ("+str(confirmedMM["Date"][-1:].to_string(index=False))+ " )")
    ...
    if graph_select == "Total Cases":
        if pc_user==True:
            ...
            total_cases_fig.add_trace(
                go.Scatter(x=confirmedMM["Date"], y=confirmedMM["Cases"], name="Total cases", showlegend=False),
                row=1,
                col=1
            )
            total_cases_fig.add_trace(
        ...
        if pc_user==True:
            ...
            recovered_cases_fig.add_trace(
                go.Scatter(x=recoveredMM["Date"], y=recoveredMM["Cases"], showlegend=False, name="Recovered", marker_color="forestgreen"),
                row=1,
                col=1
            )
            recovered_cases_fig.add_trace(
        ...
        if pc_user==True:
            ...
            death_cases_fig.add_trace(
                go.Scatter(x=deathMM["Date"], y=deathMM["Cases"], showlegend=False, name="Death", marker_color="firebrick"),
                row=1,
                col=1
            )
            death_cases_fig.add_trace(
    ...
    st.warning("Your system is not supported :(")
